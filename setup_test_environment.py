...
def check_environment() -> bool:
    ...
    if missing_vars:
        rich.print(f"[red]❌ Missing required environment variables: {missing_vars}[/red]")
...
def check_test_index(index_name: str, folder_path: str) -> bool:
    ...
    try:
        ...
        try:
            result = es.search(
                index=index_name,
                body={"query": {"match_all": {}}, "size": 1000, "_source": ["filename"]},
                timeout="30s"
            )
            indexed_files = {hit["_source"]["filename"] for hit in result["hits"]["hits"]}
            rich.print(f"[green]✅ Test index '{index_name}' exists with {len(indexed_files)} documents[/green]")
        except Exception as e:
        ...
        try:
            ...
            rich.print("[yellow]⚠️  Could not import FileHandlerRegistry, assuming all common files are supported[/yellow]")
            # Fallback: assume common extensions are supported
            supported_extensions = {'.pdf', '.txt', '.jpg', '.jpeg', '.png', '.mp3', '.mp4'}
            folder_files = set()
            ...
            rich.print(f"[blue]Found {len(folder_files)} files with common extensions in '{folder_path}'[/blue]")
        ...
        rich.print(f"[blue]Found {len(folder_files)} supported files in '{folder_path}'[/blue]")
        ...
        if missing_from_index:
            rich.print(f"[yellow]⚠️  Files in folder but not indexed: {sorted(missing_from_index)}[/yellow]")
            return False
            
        if extra_in_index:
            rich.print(f"[yellow]⚠️  Files indexed but not in folder: {sorted(extra_in_index)}[/yellow]")
            return False
...
def run_indexing(folder_path: str = "sample_folder", index_name: str = "nls_test", overwrite: bool = False) -> bool:
    """Run the indexing process using the processor with specified index."""
    try:
        rich.print(f"[blue]🔄 Starting indexing process for {folder_path} into index '{index_name}'...[/blue]")
        ...
        cmd = [sys.executable, "-m", "processor.processor", "--folder_path", folder_path]
        ...
        if result.returncode == 0:
            rich.print(f"[green]✅ Indexing completed successfully into '{index_name}'[/green]")
            return True
        else:
            rich.print(f"[red]❌ Indexing failed with exit code {result.returncode}[/red]")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Setup test environment for golden queries")
    parser.add_argument(
        "--index-folder",
    ...
    parser.add_argument(
        "--overwrite-index",
        action="store_true",
        help="Overwrite existing test index"
    )
    parser.add_argument(
        "--skip-indexing",
        action="store_true", 
    ...
    rich.print("[bold blue]🔧 Setting up test environment for golden queries[/bold blue]")
    rich.print("=" * 60)
    ...
    if not args.skip_indexing:
        ...
        if not index_ready or args.overwrite_index:
            ...
            if not run_indexing(args.index_folder, args.test_index_name, args.overwrite_index):
